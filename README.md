# 👨‍💻 Marco Miguel Borba

## Sobre mim

Olá! Meu nome é **Marco Miguel Borba**, tenho 19 anos e sou estudante de **Ciência da Computação na FURB (Universidade Regional de Blumenau)**.

Também possuo formação técnica em **Análise e Desenvolvimento de Sistemas** pelo **CEDUP Hermann Hering**.

Atualmente trabalho como **Assistente de Suporte N1 na Myrp Soluções Empresariais**, atuando com suporte a sistemas de gestão empresarial, documentos fiscais eletrônicos e atendimento técnico.

---

## 🎓 Formação

### Universidade Regional de Blumenau (FURB)

* Bacharelado em Ciência da Computação
* Em andamento

### CEDUP Hermann Hering

* Técnico em Análise e Desenvolvimento de Sistemas
* Concluído

---

## 💼 Experiência

### Assistente de Suporte N1 — Myrp Soluções Empresariais

**Outubro/2025 - Atual**

Principais atividades:

* Atendimento via chat, telefone e acesso remoto;
* Suporte a NF-e, NFC-e e MDF-e;
* Integrações com SEFAZ;
* Suporte a TEF e PinPad;
* Análise de relatórios e processos comerciais;
* Escalonamento de demandas para níveis superiores;
* Documentação de procedimentos e base de conhecimento.

---

## 🛠 Tecnologias

### Linguagens

* Java
* JavaScript
* Python
* SQL
* HTML
* CSS

### Banco de Dados

* MySQL

### Ferramentas

* Git
* GitHub
* VS Code
* Eclipse
* Workbench

---

## 📚 Áreas de Interesse

* Desenvolvimento de Software
* Banco de Dados
* Estruturas de Dados
* Algoritmos
* Suporte Técnico
* Back-end
* Sistemas Empresariais

---

## 🚀 Projetos

### Sistema de Aluguel de Hotel

Projeto acadêmico utilizando MySQL com modelagem relacional envolvendo:

* Clientes
* Funcionários
* Quartos
* Reservas
* Pagamentos
* Serviços Extras

### Validador HTML

Aplicação desenvolvida em Java para validação de estrutura e aninhamento de tags HTML.

---

## 🎮 Hobbies

* Jogos eletrônicos
* Minecraft Modded
* Hardware e montagem de computadores
* Mecânica automotiva
* Aprendizado contínuo em tecnologia

---

## 📫 Contato

* GitHub: github.com/seu-usuario
* LinkedIn: linkedin.com/in/seu-linkedin
* E-mail: [seu-email@exemplo.com](mailto:seu-email@exemplo.com)
